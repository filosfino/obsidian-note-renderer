...
- `---` 手动分页、`**加粗**` 强调色高亮
- 预设系统：保存/加载命名配置
- 单篇配置：`## renderer_config` 的 JSON/YAML 配置块覆盖全局设置，并自动写入 `rendererConfigVersion`
...
- **Per-note config**: `## renderer_config` JSON/YAML block for per-article overrides, with automatic `rendererConfigVersion`
