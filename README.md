...
- 单篇配置：优先使用 frontmatter 里的 `renderer_config`；旧的 `## renderer_config` JSON/YAML 配置块仍兼容读取
- 轻量 preset 引用：如果当前笔记仅绑定某个预设且没有 note 级覆写，frontmatter 只写 `renderer_config.presetName`
- grouped note config：只有当笔记里真的存在 note 级覆写时，才把 grouped schema 写回 frontmatter，并自动补 `rendererConfigVersion`
...
- 第二个参数：输出目录（绝对路径）
- 自动读取笔记内的 `renderer_config`；如果笔记只保存 `presetName`，会先按该 preset 解析，再叠加 note override；完全未配置时才回退到插件默认设置
- 返回输出文件路径数组
...
- 返回值始终是最新的 note-facing schema：纯 preset 引用返回 `{ presetName }`，存在 note override 时返回 grouped schema
...
- 写回时会自动校验；若只剩 preset 引用则精简落盘为 `{ presetName }`，否则统一落盘为 grouped schema 并补上 `rendererConfigVersion`
...
| 参数 | 值 |
|------|-----|
| 页面宽度 | 600px（card） / 720px（long） |
| 页面高度 | 800px（card） / 1200px（long） |
| 最终导出尺寸 | 1200×1600（card） / 1440×2400（long） |
| 上边距 | 页面高度的 7.5% → 60px（card） / 90px（long） |
| 下边距 | 页面高度的 5.625% → 45px（card） / 68px（long） |
| 左右边距 | 页面宽度的 7.5% → 45px（card） / 54px（long） |
| 内容区（long） | 612×1042px |
| 内容区（card） | 510×695px |
...
- **Per-note config**: prefer frontmatter `renderer_config` for per-article overrides; legacy `## renderer_config` JSON/YAML blocks are still readable
- **Preset reference mode**: if a note only binds to a preset and has no note-level overrides, frontmatter stores only `renderer_config.presetName`
- **Grouped note config**: when note-level overrides exist, writes use grouped frontmatter schema with automatic `rendererConfigVersion`
...
- **CLI rendering**: Render notes to PNG files via Obsidian CLI `renderToFiles` API; preset-only note configs are resolved before rendering
...
- Automatically reads the note's `renderer_config`; if the note only stores `presetName`, the preset is resolved first and note overrides are applied on top; falls back to plugin defaults only when no note config exists
...
- always returns the latest note-facing schema: preset-only notes return `{ presetName }`, notes with overrides return grouped schema
...
- validates input; writes `{ presetName }` when only a preset reference remains, otherwise adds `rendererConfigVersion` and writes grouped schema back to the note
...
| Parameter | Value |
|-----------|-------|
| Page width | 600px (card) / 720px (long) |
| Page height | 800px (card) / 1200px (long) |
| Final export size | 1200×1600 (card) / 1440×2400 (long) |
| Top padding | 7.5% of page height → 60px (card) / 90px (long) |
| Bottom padding | 5.625% of page height → 45px (card) / 68px (long) |
| Horizontal padding | 7.5% of page width → 45px (card) / 54px (long) |
