...
- 字号调整（20-30px）、50+ 中文字体可选（黑体、宋体、仿宋、楷体、圆体）
- 封面字体独立选择
...
- working config 仍存在于插件侧；切换 note 时会回到默认工作态，但 note frontmatter 支持保存轻量 `renderer_config`：`presetName` 或 `presetName + 局部 override`
...
更多可复制到 vault 的完整示例见 [`examples/`](examples/)。
...
- **Working config workflow**: rendering state lives in the plugin, while note frontmatter can store lightweight `renderer_config` as `presetName` or `presetName + partial overrides`
...
For complete notes you can copy into a vault, see [`examples/`](examples/).
