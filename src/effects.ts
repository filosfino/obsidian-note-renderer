...
function createOverlay(cssText: string): HTMLElement {
  const div = document.createElement("div");
  div.style.cssText = `position:absolute;top:0;left:0;width:100%;height:100%;pointer-events:none;z-index:2;${cssText}`;
  return div;
}
...
function renderGrain(container: HTMLElement, params: EffectParams): void {
  ...
  const el = createOverlay(`opacity:${op};mix-blend-mode:overlay;`);
...
function renderAurora(container: HTMLElement, params: EffectParams, ctx: EffectContext): void {
  ...
  const el = createOverlay(`opacity:${op};mix-blend-mode:${ctx.effectBlend};overflow:hidden;`);
  el.innerHTML = `
...
function renderBokeh(container: HTMLElement, params: EffectParams, ctx: EffectContext): void {
  const op = params.opacity / 100;
  const el = createOverlay(`opacity:${op};`);
...
function renderGrid(container: HTMLElement, params: EffectParams, ctx: EffectContext): void {
  ...
  const el = createOverlay(`opacity:${op};background-image:repeating-linear-gradient(0deg,${lineColor} 0px,${lineColor} 1px,transparent 1px,transparent 60px),repeating-linear-gradient(90deg,${lineColor} 0px,${lineColor} 1px,transparent 1px,transparent 60px);`);
  container.appendChild(el);
}
...
function renderScanlines(container: HTMLElement, params: EffectParams, ctx: EffectContext): void {
  ...
  const el = createOverlay(`opacity:${op};background:repeating-linear-gradient(to bottom,transparent 0px,transparent 2px,${scanColor} 2px,${scanColor} 4px);`);
  container.appendChild(el);
}
...
function renderNetwork(container: HTMLElement, params: EffectParams, ctx: EffectContext): void {
  ...
  const el = createOverlay(`opacity:${op};color:${netColor};`);
